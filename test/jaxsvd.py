...
def compress_image(input_filename, output_folder, k):
	...
	for i,channel in enumerate(channels):
		...
		channel_u, channel_s, channel_v = jnp.linalg.svd(channel, full_matrices=False)
		channel_u_np = np.array(channel_u)
		channel_s_np = np.array(channel_s)
		channel_v_np = np.array(channel_v)
		print(type(channel_u))
		svd_end_time = time.time()
		print(f"SVD computation time: {svd_end_time - svd_start_time}")
		mat_mul_time_start = time.time()
		print("Hello, it will crash on the next line")
		k_rank_approx = channel_u_np[:, 0:k].dot(np.diag(channel_s_np[0:k]).dot(channel_v_np[0:k,:]))
